...
function KellyColorPicker(cfg) {
    ...
    // style switch from triangle to quad and backwards
    ...
    function initColorSaver(align, selected, color) {
        ...
        console.log("colorSaverKey: "+colorSaverKey);
    ...
    wheelCursor.path; // rotatePath2 --- rotation according to the old function, the center is not added in the figure
    ...
    // svCursorMouse - for devices with a mouse, pointer generator depending on the active area
    ...
    // may be zero in some cases / check before apply
        ...
        if (typeof cString == 'string') {
            ...
            if (cString.indexOf("(") == -1) { // hex color
                ...
                if (cString.length >= 3) {
                    ...
                    // complete full format, by replicating the R, G, and B values 
    ...
    function getSvFigureQuad() {
        ...
        quad.size;
        quad.padding = 2;
        quad.path; // extreme points of the figure on the coordinate plane
        quad.imageData = null; // rendered quad image data
        // overwrites the existing one so as not to cause memory leaks, nullifying the past
        // mk UInt8ClampedArray generated by createImageData in standard ways
        // in all browsers is unloaded at once
        ...
        quad.updateSize = function () {
            ...
            // based on the formula for the diagonal of the square, we find out the length of the side based on the available diameter
            this.size = Math.floor(workD / Math.sqrt(2));

            this.path = new Array();

            // we find the upper left point and from it we set the rest of the coordinates
    ...
    function getSvFigureTriangle() {
        ...
        triangle.size; // side of an equilateral triangle
        triangle.padding = 2;
        triangle.path;
        ...
        triangle.updateSize = function () {
            // from the formula for the height of an equilateral triangle
            this.outerRadius = wheel.innerRadius - wheelCursor.paddingX - this.padding;
            // from the triangle sine theorem
    ...
    // rotation of a figure about a point
    ...
    function updateColorSavers() {
        ...
        console.log("hex in updateColorSavers: "+JSON.stringify(hex));
        }
    ...
    // display interface without cursors
    // buffering supported todo add buffering color saver items
    // move the alpha slider buffer separately from the wheel and sv block
    ...
    function drawColorPicker() {
        ...
        // the shape of the ring can only change when the widget is resized
        wheel.draw();
        svFig.draw();
        ...
        // put the currently rendered image of the ring + sv selector into the buffer
        // notice :

        // when you move the cursor of the ring, it makes no sense to save the buffer for the entire image - the sv block is constantly updated, so
        // save already at the exit event from the move process
    ...
    this.setHueByDot = function (dot) {
        ...
        thisColor = hsv.h+','+hsv.s+','+hsv.v;
        //console.log("rgb stringify: "+JSON.stringify(thisColor));
        //console.log("rgb stringify: "+JSON.stringify(thisColor));
        absoluteColor = hex;
        sendColor(hex);
        // sendColor("#a4373e");
    ...
    this.setColorByDot = function (dot) {
        // console.log("dot in setColorByDot: "+JSON.stringify(dot));
        var sv = svFig.dotToSv(dot);
        // console.log("sv in setColorByDot: "+JSON.stringify(sv));
        ...
        rgb = hsvToRgb(hsv.h, hsv.s, hsv.v);
        hex = rgbToHex(rgb);
        // console.log("rgb in setColorByDot: "+JSON.stringify(rgb));
        // console.log("hex in setColorByDot: "+JSON.stringify(hex));
        // console.log("colorSavers in setColorByDot: "+JSON.stringify(colorSavers));
    ...
    // move the pointer over the canvas at rest
    ...
    // in FireFox for android there is a "feature" that hides or reveals the address bar toolbar when you move your finger
    // you can disable it only through the about: config option browser.chrome.dynamictoolbar
    ...
    this.mouseDownEvent = function (event) {
        ...
        if (wheel.isDotIn(newDot)) {
            // console.log("newDot in wheel: "+JSON.stringify(newDot));
            drag = 'wheel';
            ...
            // console.log("newDot in svFig: "+JSON.stringify(newDot));
            drag = 'sv';
    ...
    // to customize the display of widget items
    ...
    // if size same as current and refresh variable setting to true - refresh current view anyway
    // otherwise exit with return true
    ...
    this.destroy = function () {
        ...
        // remove popup close and resize events if this picker include them earlier
...
/**
 * Trigger for KellyColorPicker objects, so as not to reset the cursor style on hover if there is already a color selection
 * Notice: when the cursor goes beyond the bounds of the current canvas, the event of an unknown object can always reset the cursor image
 */

KellyColorPicker.cursorLock = false; // you can specify the handler of the object
KellyColorPicker.activePopUp = false;
